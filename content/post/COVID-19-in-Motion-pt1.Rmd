---
title: "World Data Overview - Global Heat Maps"
output:
  self_contained: FALSE
  blogdown::html_page:
    dev: "svg"
  keep_md: TRUE
author: "Houston Haynes"
date: '2020-06-28T14:59:43+05:30'
featuredpath: img
featured: COVID-19.png
ogfeatured: H3_og_wide_covid19.png
emblem: health.png
description: COVID-19 in Motion - Animated Time Series for cases, deaths and testing
draft: false
tags:
- COVID-19
categories: 
- Deep Dive
editor_options: 
  chunk_output_type: inline
---
```{r setup and file loads, include=FALSE}
knitr::opts_chunk$set(
	echo = FALSE,
	message = FALSE,
	warning = FALSE,
	widgetframe_widgetsdir = 'my_widgets'
)

library(highcharter)
library(tidyverse)
library(lubridate)
library(widgetframe)
library(xts)
library(jsonlite)
library(fontawesome)

URL <- "https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/owid-covid-data.csv"

coviddata <- read.csv(URL)

thm <- hc_theme_merge(
  hc_theme_ffx(),
  hc_theme(title = list(style = list(fontFamily = "Ubuntu")),
           subtitle = list(style = list(fontFamily = "Fira Code")),
           legenc = list(itemStyle = list(fontFamily ='Ubuntu'))))

```

```{r Tweets_from_Faces_of_COVID, include=FALSE }

library(rtweet)
library(stringr)
library(dplyr)

appname <- "H3 Tech Blog"
contumer_key <- "Tb5d892Ozn7W3EP2uvm5RVQPH"
contumer_secret <- "i094J3ewdwxm1KKmY3n4hnJ5ygxwwrl2cjvcfYWCv4VjQwOHHh"
access_key <- "4434818952-MGJ5dPTsqmtm5sQHR3w2ebpDVt14wYsQeWBN9Bw"
access_secret <- "p5sI2H0Z8YeZ8Tiyvv8ivb6R6fYmPP7VgqoNkjEGgDBUm"

token <- create_token(
  appname, 
  contumer_key, 
  contumer_secret,
  access_key,
  access_secret)

COVIDtweets <- get_timeline("FacesOfCOVID", n = 1000, parse = TRUE, include_rts = FALSE, ) |>
  filter(str_detect(text, 'died of COVID'))

COVIDtweets <- sample(COVIDtweets$status_id)



```

# Exercise in Scale

When I started working with the <a href="https://ourworldindata.org/" target="_blank">Our World In Data</a> COVID-19 data set I wanted to chart out a few things to get a *feel* for the data. I've done this work for quite a while and the first thing you often do is a form of *reality* check. In some cases I checked against other charts based on the same data and thought "that can't be right" only to plot it myself and see it directly. (looking at you, *Europe*) As we all have witnessed this year experts have been sounding the alarm since early on. Pandemics are like locomotives, they're difficult to stop once they gain momentum. The same goes for an effective response.

`r blogdown::shortcode("fancybox-iframe", "/post/COVID-19-in-Motion-pt1_files/figure-html/widgets/widget_New_Cases_by_Continent.html")`

```{r New_Cases_by_Continent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE  }

library(zoo)
library(slider)
library(DT)
library(highcharter)

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)

data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, continent, location, new_cases)   |>
  filter(location != "International",
         location != "World",
         date > "2020-01-23") |>
  mutate(new_cases = ifelse(is.na(new_cases), 0, new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_cases == 1057),
                             57,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_cases == 3887),
                             187,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_cases == 3935),
                             235,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_cases == 3800),
                             38,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_cases == 1610),
                             61,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_cases == 3351),
                            351,
                            new_cases
                            )) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_cases == 398),
                            39,
                            new_cases
                            )) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_cases == 3013),
                            313,
                            new_cases
                            )) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, continent, new_cases, population) |>
  group_by(date, continent) |>
  summarize(new_cases = sum(new_cases), population = sum(population)) |>
  mutate(ncpm = ((new_cases / population) * 1000000)) |>
  group_by(date, continent) |>
  summarize(new_cases = sum(new_cases), ncpm = sum(ncpm)) |>
  group_by(continent) |>
  arrange(continent, date) |>
  mutate(ncrm = slider::slide_dbl(new_cases, mean, .before = 3, .after = 3)) |>
  mutate(ncpmrm = slider::slide_dbl(ncpm, mean, .before = 3, .after = 3)) |>
  ungroup()

widget <- hchart(data, "area", hcaes(x = date, y = ncpmrm, group = continent, labels = FALSE),
      tooltip = list(
        pointFormat = "{series.name}: {point.ncpmrm:.2f}")) |>
  hc_plotOptions(series = list(stacking = 'normal')) |>
  hc_title(text = "World COVID Daily New Cases per Million (smoothed)", align = "center") |>
  hc_add_theme(thm) |>
  hc_subtitle(text = "Summed by continent :: Higher == BAD", align = "center")|>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_yAxis(title = list(enabled = FALSE)) |>
  hc_xAxis(title = list(enabled = FALSE))

frameWidget(widget, width="100%", height="26rem")

```

`r blogdown::shortcode("alert", "info", "Working with the Report:", "Use the legend to highlight, show or hide a series. Hovering over bands in the chart reveals detail for a given date.")`

So I took the data and plotted it on a map, to play on a day grain through time. The gradients are pretty subtle at the start, and that's because of the data getting *very* dark late in the range.

`r blogdown::shortcode("fancybox-iframe", "/post/COVID-19-in-Motion-pt1_files/figure-html/widgets/widget_Global_New_Cases_Per_Million.html")`

```{r Global_New_Cases_Per_Million, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

library(ggplot2)

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, iso_code, location, new_cases)   |>
  filter(location != "International",
         location != "World") |>
  mutate(new_cases = ifelse(is.na(new_cases), 0, new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_cases == 1057),
                             57,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_cases == 3887),
                             187,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_cases == 3935),
                             235,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_cases == 3800),
                             38,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_cases == 1610),
                             61,
                             new_cases
  )) |>
  mutate(new_cases = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_cases == 3351),
                            351,
                            new_cases
                            )) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_cases == 398),
                            39,
                            new_cases
                            )) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_cases == 3013),
                            313,
                            new_cases
                            )) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, iso_code, new_cases, population) |>
  group_by(date, iso_code, population) |>
  summarize(new_cases = sum(new_cases)) |>
  mutate(ncpm = ((new_cases / population) * 1000000)) |>
  group_by(iso_code) |>
  arrange(iso_code, date) |>
  mutate(ncrm = slider::slide_dbl(new_cases, mean, .before = 3, .after = 3)) |>
  mutate(ncpmrm = slider::slide_dbl(ncpm, mean, .before = 3, .after = 3)) |>
  ungroup() |>
  mutate_at(vars(ncpm, ncrm, ncpmrm), funs(round(., 2)))

#datatable(data)


ds <- data %>%
  group_by(iso_code) %>%
  do(item = list(
    iso_code = first(.$iso_code),
    sequence = .$ncpmrm,
    ncpmrm = last(.$ncpmrm))) %>%
  .$item

#head(ds, 10)

vec <- sort(unique(pull(data, date)))

url <- "https://code.highcharts.com/mapdata/custom/world-robinson-highres.js"
tmpfile <- tempfile(fileext = ".json")
download.file(url, tmpfile)
geo <- readLines(tmpfile)
geo <- gsub(".* = ", "", geo)
map <- jsonlite::fromJSON(geo, simplifyVector = FALSE)

last_updated <- paste("Source: https://ourworldincata.org/  -  Report Last Updated:",  format(Sys.time(), "%a %b %d %Y %X"))

widget <- highchart(type = "map") |>
  hc_add_theme(thm) |>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_title(text = "Global COVID Daily New Cases per Million (smoothed)", align = "center") |>
  hc_subtitle(text = "Press play button or use slider to change date", align = "center") |>
  hc_add_series(data = ds,
                name = "New Cases per Million",
                mapData = map,
                joinBy = c("iso-a3", "iso_code"),
                borderWidth = .25,
                borderColor = 'rgba(160, 160, 160, 0.9)') |>
  hc_colorAxis (min = 0 ,
                max = 1000,
                stops = color_stops(colors = viridisLite::inferno(20,
                                                                begin = 0.1,
                                                                direction = -1))) |>
  hc_motion (
    enabled = TRUE,
    startIndex = 23,
    axisLabel = "date",
    labels = vec,
    updateInterval = 5,
    playIcon = "fas fa-play",
    pauseIcon = "fas fa-pause",
    magnet = list(
      rounc = "floor",
      step = 0.1
      )
  ) |>
  hc_legend(layout = "vertical", verticalAlign = "top",
            align = "right", valueDecimals = 0) |>
  hc_mapNavigation(
    enabled = TRUE,
    enableMouseWheelZoom = TRUE,
    enableDoubleClickZoom = TRUE
  ) |>
  hc_credits(enabled = TRUE,
             text = last_updated,
             position = list(align = "left", x = 10, y = -5))|>
  hc_legend(enabled = FALSE) 


frameWidget(widget, width="100%", height="40rem")

```

`r blogdown::shortcode("alert", "info", "Working with the report:", "Aside from the play/pause and date slider, the map is *zoomable*. Hovering over a country will highlight and show the value for the current position on the timeline. Hovering while playing back will show updating changes to the values for that country.")`

`r blogdown::shortcode("alert", "warning", "Bug Report:", "The play/pause button for the map charts *works* but the icons are currently not visible. A fix is being researched now. Thanks for understanding.")`

# Daily COVID Mortality

One of the things I expected was to see some "chop" in early data as reporting became more consistent. Here I was surprised to see irregular bumps in the "smoothed" data. Once South America is removed from the set then things look much less strange. Such as it is, the visuals tell a convincing story of where things are going as of November, and it's *not* good.

`r blogdown::shortcode("fancybox-iframe", "/post/COVID-19-in-Motion-pt1_files/figure-html/widgets/widget_New_Deaths_by_Continent.html")`

```{r New_Deaths_by_Continent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE  }

library(zoo)
library(slider)
library(DT)
library(highcharter)

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, continent, location, new_deaths)   |>
  filter(location != "International",
         location != "World",
         date > "2020-01-23") |>
  mutate(new_deaths = ifelse(is.na(new_deaths), 0, new_deaths)) |>
  mutate(new_deaths = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_deaths == 1057),
                             57,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_deaths == 3887),
                             187,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_deaths == 3935),
                             235,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_deaths == 3800),
                             38,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_deaths == 1610),
                             61,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_deaths == 3351),
                            351,
                            new_deaths
                            )) |>
  mutate(new_deaths = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_deaths == 398),
                            39,
                            new_deaths
                            )) |>
  mutate(new_deaths = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_deaths == 3013),
                            313,
                            new_deaths
                            )) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, continent, new_deaths, population) |>
  group_by(date, continent) |>
  summarize(new_deaths = sum(new_deaths), population = sum(population)) |>
  mutate(ndpm = ((new_deaths / population) * 1000000)) |>
  group_by(date, continent) |>
  summarize(new_deaths = sum(new_deaths), ndpm = sum(ndpm)) |>
  group_by(continent) |>
  arrange(continent, date) |>
  mutate(ndrm = slider::slide_dbl(new_deaths, mean, .before = 3, .after = 3)) |>
  mutate(ndpmrm = slider::slide_dbl(ndpm, mean, .before = 3, .after = 3)) |>
  ungroup()

#datatable(data)

widget <- hchart(data, "area", hcaes(x = date, y = ndpmrm, group = continent, labels = FALSE)) |>
  hc_plotOptions(series = list(stacking = 'normal')) |>
  hc_title(text = "World COVID Daily New Deaths per Million (smoothed)", align = "center") |>
  hc_add_theme(thm) |>
  hc_subtitle(text = "Summed by continent :: Higher == BAD", align = "center")|>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_yAxis(title = list(enabled = FALSE)) |>
  hc_xAxis(title = list(enabled = FALSE))

frameWidget(widget, width="100%", height="26rem")

```

Again here - like the area chart - the direction of what's happening in Europe is unmistakable. In fact it bothers me to the point that I'll probably end up sourcing some other data just so I can sleep tonight. But one thing we can see and possibly discard is the sudden jump in values in South America. Per our area chart above which showed that as the offending portion of the data set that was skewing the result.
 
`r blogdown::shortcode("fancybox-iframe", "/post/COVID-19-in-Motion-pt1_files/figure-html/widgets/widget_Global_New_Deaths_Per_Million.html")` 
 
```{r Global_New_Deaths_Per_Million, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

op <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, iso_code, location, new_deaths)   |>
  filter(location != "International",
         location != "World") |>
  mutate(new_deaths = ifelse(is.na(new_deaths), 0, new_deaths)) |>
  mutate(new_deaths = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_deaths == 1057),
                             57,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_deaths == 3887),
                             187,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_deaths == 3935),
                             235,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_deaths == 3800),
                             38,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_deaths == 1610),
                             61,
                             new_deaths
  )) |>
  mutate(new_deaths = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_deaths == 3351),
                            351,
                            new_deaths
                            )) |>
  mutate(new_deaths = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_deaths == 398),
                            39,
                            new_deaths
                            )) |>
  mutate(new_deaths = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_deaths == 3013),
                            313,
                            new_deaths
                            )) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, iso_code, new_deaths, population) |>
  group_by(date, iso_code, population) |>
  summarize(new_deaths = sum(new_deaths)) |>
  mutate(ndpm = ((new_deaths / population) * 1000000)) |>
  group_by(iso_code) |>
  arrange(iso_code, date) |>
  mutate(ndrm = slider::slide_dbl(new_deaths, mean, .before = 3, .after = 3)) |>
  mutate(ndpmrm = slider::slide_dbl(ndpm, mean, .before = 3, .after = 3)) |>
  ungroup() |>
  mutate_at(vars(ndpm, ndrm, ndpmrm), funs(round(., 2)))

#datatable(data)


ds <- data %>%
  group_by(iso_code) %>%
  do(item = list(
    iso_code = first(.$iso_code),
    sequence = .$ndpmrm,
    ndpmrm = last(.$ndpmrm))) %>%
  .$item

#head(ds, 10)

vec <- sort(unique(pull(data, date)))

url <- "https://code.highcharts.com/mapdata/custom/world-robinson-highres.js"
tmpfile <- tempfile(fileext = ".json")
download.file(url, tmpfile)
geo <- readLines(tmpfile)
geo <- gsub(".* = ", "", geo)
map <- jsonlite::fromJSON(geo, simplifyVector = FALSE)

last_updated <- paste("Source: https://ourworldindata.org/  -  Report Last Updated:",  format(Sys.time(), "%a %b %d %Y %X"))

widget <- highchart(type = "map") |>
  hc_add_theme(thm) |>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_title(text = "Global COVID Daily New Deaths per Million (smoothed)", align = "center") |>
  hc_subtitle(text = "Press play button or use slider to change date", align = "center") |>
  hc_add_series(data = ds,
                name = "New Cases per Million",
                mapData = map,
                joinBy = c("iso-a3", "iso_code"),
                borderWidth = .25,
                borderColor = 'rgba(160, 160, 160, 0.9)') |>
  hc_colorAxis (min = 0 ,
                max = 15,
                stops = color_stops(colors = viridisLite::inferno(20,
                                                                begin = 0.1,
                                                                direction = -1))) |>
  hc_motion (
    enabled = TRUE,
    startIndex = 23,
    axisLabel = "date",
    labels = vec,
    updateInterval = 5,
    playIcon = "fas fa-play",
    pauseIcon = "fas fa-pause",
    magnet = list(
      round = "floor",
      step = 0.1
      )
  ) |>
  hc_legend(layout = "vertical", verticalAlign = "top",
            align = "right", valueDecimals = 0) |>
  hc_mapNavigation(
    enabled = TRUE,
    enableMouseWheelZoom = TRUE,
    enableDoubleClickZoom = TRUE
  ) |>
  hc_credits(enabled = TRUE,
             text = last_updated,
             position = list(align = "left", x = 10, y = -5))


frameWidget(widget, width="100%", height="40rem")

```

<div class="separator">Faces of COVID-19</div>
<figure>
<div class="row">
  <div class="col-md-4">`r blogdown::shortcode("tweet", COVIDtweets[1])`</div>
  <div class="col-md-4">`r blogdown::shortcode("tweet", COVIDtweets[2])`</div>
  <div class="col-md-4">`r blogdown::shortcode("tweet", COVIDtweets[3])`</div>
</div>
<figcaption class="caption">The pandemic-related posts on this site are about more than data. <strong>Behind every number is a person, a family and a community.</strong> As reports are refreshed, new selectiont will also be chosen at random. To see how this is done, <a href="/post/faces-of-covid">see this sidebar</a>.</figcaption>
<hr>
</figure>

# Testing, Testing, 1, 2, 3...

While spelunking through the data I thought I'd take a look at testing. The good news, the numbers are increasing. 

`r blogdown::shortcode("fancybox-iframe", "/post/COVID-19-in-Motion-pt1_files/figure-html/widgets/widget_New_Tests_by_Continent.html")`

```{r New_Tests_by_Continent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE  }

library(zoo)
library(slider)
library(DT)
library(highcharter)

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, continent, location, new_tests)   |>
  filter(location != "International",
         location != "World",
         date > "2020-01-23") |>
  mutate(new_tests = ifelse(is.na(new_tests), 0, new_tests)) |>
  mutate(new_tests = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_tests == 1057),
                             57,
                             new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_tests == 3887),
                             187,
                             new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_tests == 3935),
                             235,
                             new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_tests == 3800),
                             38,
                             new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_tests == 1610),
                             61,
                             new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_tests == 3351),
                            351,
                            new_tests
                            )) |>
  mutate(new_tests = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_tests == 398),
                            39,
                            new_tests
                            )) |>
  mutate(new_tests = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_tests == 3013),
                            313,
                            new_tests
                            )) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, continent, new_tests, population) |>
  group_by(date, continent) |>
  summarize(new_tests = sum(new_tests), population = sum(population)) |>
  mutate(ntpt = ((new_tests / population) * 1000)) |>
  group_by(date, continent) |>
  summarize(new_tests = sum(new_tests), ntpt = sum(ntpt)) |>
  group_by(continent) |>
  arrange(continent, date) |>
  mutate(ntrm = slider::slide_dbl(new_tests, mean, .before = 3, .after = 3)) |>
  mutate(ntptrm = slider::slide_dbl(ntpt, mean, .before = 3, .after = 3)) |>
  ungroup()

#datatable(data)

widget <- hchart(data, "area", hcaes(x = date, y = ntptrm, group = continent, labels = FALSE)) |>
  hc_plotOptions(series = list(stacking = 'normal')) |>
  hc_title(text = "World COVID Daily New Tests per Thousand (smoothed)", align = "center") |>
  hc_add_theme(thm) |>
  hc_subtitle(text = "Summed by continent :: Higher == GOOD", align = "center")|>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_yAxis(title = list(enabled = FALSE)) |>
  hc_xAxis(title = list(enabled = FALSE))

frameWidget(widget, width="100%", height="26rem")

```

Here's the flip-side of the previous two reports - where the numbers go *green* as they go higher. But again, it runt the risk of painting an inaccurate picture to show  less than 5 tests per 1,000 people as a positive indication. That value should be much, much higher - but in this case relative values matter. 

`r blogdown::shortcode("fancybox-iframe", "/post/COVID-19-in-Motion-pt1_files/figure-html/widgets/widget_Global_New_Tests_Per_Thousand.html")` 

```{r Global_New_Tests_Per_Thousand, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)), ],
               date, iso_code, location, new_tests)   |>
  filter(location != "International",
         location != "World") |>
  mutate(new_tests = ifelse(is.na(new_tests), 0, new_tests)) |>
  mutate(new_tests = ifelse((date == "2020-07-18" &
                               location == "Chile" &
                               new_tests == 1057),
                            57,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-07-24" &
                               location == "Peru" &
                               new_tests == 3887),
                            187,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-08-14" &
                               location == "Peru" &
                               new_tests == 3935),
                            235,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-09-07" &
                               location == "Ecuador" &
                               new_tests == 3800),
                            38,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-09-07" &
                               location == "Bolivia" &
                               new_tests == 1610),
                            61,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-10-02" &
                               location == "Argentina" &
                               new_tests == 3351),
                            351,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-10-09" &
                               location == "Ecuador" &
                               new_tests == 398),
                            39,
                            new_tests
  )) |>
  mutate(new_tests = ifelse((date == "2020-10-09" &
                               location == "Mexico" &
                               new_tests == 3013),
                            313,
                            new_tests
  )) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, iso_code, new_tests, population) |>
  group_by(date, iso_code, population) |>
  summarize(new_tests = sum(new_tests)) |>
  mutate(ntpt = ((new_tests / population) * 1000)) |>
  group_by(iso_code) |>
  arrange(iso_code, date) |>
  mutate(ntrm = slider::slide_dbl(new_tests, mean, .before = 3, .after = 3)) |>
  mutate(ntptrm = slider::slide_dbl(ntpt, mean, .before = 3, .after = 3)) |>
  ungroup() |>
  mutate_at(vars(ntpt, ntrm, ntptrm), funs(round(., 2)))

ds <- data %>%
  group_by(iso_code) %>%
  do(item = list(
    iso_code = first(.$iso_code),
    sequence = .$ntptrm,
    ndpmrm = last(.$ntptrm))) %>%
  .$item

#head(ds, 10)

vec <- sort(unique(pull(data, date)))

url <- "https://code.highcharts.com/mapdata/custom/world-robinson-highres.js"
tmpfile <- tempfile(fileext = ".json")
download.file(url, tmpfile)
geo <- readLines(tmpfile)
geo <- gsub(".* = ", "", geo)
map <- jsonlite::fromJSON(geo, simplifyVector = FALSE)

last_updated <- paste("Source: https://ourworldindata.org/  -  Report Last Updated:",  format(Sys.time(), "%a %b %d %Y %X"))

widget <- highchart(type = "map") |>
  hc_add_theme(thm) |>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_title(text = "Global COVID Daily New Tests per Thousand (smoothed)", align = "center") |>
  hc_subtitle(text = "Press play button or use slider to change date", align = "center") |>
  hc_add_series(data = ds,
                name = "New Cases per Million",
                mapData = map,
                joinBy = c("iso-a3", "iso_code"),
                borderWidth = .25,
                borderColor = 'rgba(160, 160, 160, 0.9)') |>
  hc_colorAxis (min = 0 ,
                max = 5,
                stops = color_stops(colors = viridisLite::viridis(4,
                                                                begin = 0.1,
                                                                direction = -1))) |>
  hc_motion (
    enabled = TRUE,
    startIndex = 23,
    axisLabel = "date",
    labels = vec,
    updateInterval = 5,
    playIcon = "fas fa-play",
    pauseIcon = "fas fa-pause",
    magnet = list(
      round = "floor",
      step = 0.1
      )
  ) |>
  hc_legend(layout = "vertical", verticalAlign = "top",
            align = "right", valueDecimals = 0) |>
  hc_mapNavigation(
    enabled = TRUE,
    enableMouseWheelZoom = TRUE,
    enableDoubleClickZoom = TRUE
  ) |>
  hc_credits(enabled = TRUE,
             text = last_updated,
             position = list(align = "left", x = 10, y = -5))

frameWidget(widget, width="100%", height="40rem")

```

## How the reports were built

The area charts are relatively straightforward. The only data munging task was summing the values by continent and date. After that it was mainly about formatting and some tweaks to keep the display clean. 

<button class="btn btn-outline-dark btn-sm mt-2 mb-2" data-toggle="collapse" data-target="#code1">View the Code behind the First Area Chart</button>

<pre id="code1" class="collapse"><code class="r"># begin setup code chunk
library(highcharter)
library(widgetframe)
library(tidyverse)
library(lubridate)
library(xts)

URL <- "https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/owid-covid-data.csv"

coviddata <- read.csv(URL)

# end setup code chunk

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, continent, location, new_cases)   |>
  filter(location != "International",
         location != "World",
         date > "2020-03-01") |>
  mutate(new_cases = ifelse(is.na(new_cases), 0, new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_cases == 1057),
                             57,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_cases == 3887),
                             187,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_cases == 3935),
                             235,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_cases == 3800),
                             38,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_cases == 1610),
                             61,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_cases == 3351),
                            351,
                            new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_cases == 398),
                            39,
                            new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_cases == 3013),
                            313,
                            new_cases)) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, continent, new_cases, population) |>
  group_by(date, continent) |>
  summarize(new_cases = sum(new_cases), population = sum(population)) |>
  mutate(ncpm = ((new_cases / population) * 1000000)) |>
  group_by(date, continent) |>
  summarize(new_cases = sum(new_cases), ncpm = sum(ncpm)) |>
  group_by(continent) |>
  arrange(continent, date) |>
  mutate(ncrm = slider::slide_dbl(new_cases, mean, .before = 3, .after = 3)) |>
  mutate(ncpmrm = slider::slide_dbl(ncpm, mean, .before = 3, .after = 3)) |>
  ungroup()

#datatable(data)

thm <- hc_theme_merge(
  hc_theme_ffx(),
  hc_theme(title = list(style = list(fontFamily = "Ubuntu")),
           subtitle = list(style = list(fontFamily = "Fira Code")),
           legenc = list(itemStyle = list(fontFamily ='Ubuntu'))))

widget <- hchart(data, "area", hcaes(x = date, y = ncpmrm, group = continent, labels = FALSE)) |>
  hc_plotOptions(series = list(stacking = 'normal')) |>
  hc_title(text = "World COVID Daily New Cases per Million (smoothed)", align = "center") |>
  hc_add_theme(thm) |>
  hc_subtitle(text = "Summed by continent :: Higher == BAD", align = "center")|>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_yAxis(title = list(enabled = FALSE)) |>
  hc_xAxis(title = list(enabled = FALSE))

frameWidget(widget, width="100%", height="26rem")</code></pre>

The map charts were more complex. Aside from tying together the geojson map to the data (by joining iso-a3 in the map data to the iso_code value from the source data) there was building the sequence of data itself. This wasn't immediately apparent but <a href="https://jkuntt.com/blog/posts/2016-04-12-adding-motion-to-choropleths/" target="_blank">this tutorial</a> was a great help.

<button class="btn btn-outline-dark btn-sm mt-2 mb-2" data-toggle="collapse" data-target="#code2">View the Code behind the First Map Chart</button>

<pre id="code2" class="collapse"><code class="r"># begin setup code chunk
library(highcharter)
library(widgetframe)
library(tidyverse)
library(lubridate)
library(xts)

URL <- "https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/owid-covid-data.csv"

coviddata <- read.csv(URL)

# end setup code chunk

pop <- read.csv("../../data/population_by_country_2020.csv", header = TRUE)
data <- select(coviddata[with(coviddata, order(date, iso_code)),], 
               date, iso_code, location, new_cases)   |>
  filter(location != "International",
         location != "World") |>
  mutate(new_cases = ifelse(is.na(new_cases), 0, new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-07-18" &
                                location == "Chile" &
                                new_cases == 1057),
                             57,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-07-24" &
                                location == "Peru" &
                                new_cases == 3887),
                             187,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-08-14" &
                                location == "Peru" &
                                new_cases == 3935),
                             235,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Ecuador" &
                                new_cases == 3800),
                             38,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-09-07" &
                                location == "Bolivia" &
                                new_cases == 1610),
                             61,
                             new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-10-02" &
                                location == "Argentina" &
                                new_cases == 3351),
                            351,
                            new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Ecuador" &
                                new_cases == 398),
                            39,
                            new_cases)) |>
  mutate(new_cases = ifelse((date == "2020-10-09" &
                                location == "Mexico" &
                                new_cases == 3013),
                            313,
                            new_cases)) |>
  mutate(date = ymd(date)) |>
  left_join(pop, location = location) |>
  select(date, iso_code, new_cases, population) |>
  group_by(date, iso_code, population) |>
  summarize(new_cases = sum(new_cases)) |>
  mutate(ncpm = ((new_cases / population) * 1000000)) |>
  group_by(iso_code) |>
  arrange(iso_code, date) |>
  mutate(ncrm = slider::slide_dbl(new_cases, mean, .before = 3, .after = 3)) |>
  mutate(ncpmrm = slider::slide_dbl(ncpm, mean, .before = 3, .after = 3)) |>
  ungroup() |>
  mutate_at(vars(ncpm, ncrm, ncpmrm), funs(round(., 2)))

#datatable(data)


ds <- data %>%
  group_by(iso_code) %>%
  do(item = list(
    iso_code = first(.$iso_code),
    sequence = .$ntptrm,
    ndpmrm = last(.$ntptrm))) %>%
  .$item

#head(ds, 10)

vec <- sort(unique(pull(data, date)))

url <- "https://code.highcharts.com/mapdata/custom/world-robinson-highres.js"
tmpfile <- tempfile(fileext = ".json")
download.file(url, tmpfile)
geo <- readLines(tmpfile)
geo <- gsub(".* = ", "", geo)
map <- jsonlite::fromJSON(geo, simplifyVector = FALSE)

last_updated <- paste("Source: https://ourworldincata.org/  -  Report Last Updated:",
                      format(Sys.time(), "%a %b %d %Y %X"))

thm <- hc_theme_merge(
  hc_theme_ffx(),
  hc_theme(title = list(style = list(fontFamily = "Ubuntu")),
           subtitle = list(style = list(fontFamily = "Ubuntu")),
           legenc = list(itemStyle = list(fontFamily ='Ubuntu'))))

widget <- highchart(type = "map") |>
  hc_add_theme(thm) |>
  hc_chart(
    borderColor = 'rgba(160, 160, 160, 0.3)',
    borderRadius = 8,
    borderWidth = 2,
    marginBottom = '80',
    marginTop = '60',
    marginLeft = '60',
    marginRight = '60') |>
  hc_title(text = "Global COVID Daily New Cases per Million (smoothed)", align = "center") |>
  hc_subtitle(text = "Press play button or use slider to change date", align = "center") |>
  hc_add_series(data = ds,
                name = "New Cases per Million",
                mapData = map,
                joinBy = c("iso-a3", "iso_code"),
                borderWidth = 0.01) |>
  hc_colorAxis (min = 0 ,
                max = 1000,
                stops = color_stops(colors = viridisLite::inferno(20,
                                                                begin = 0.1,
                                                                direction = -1))) |>
  hc_motion (
    enabled = TRUE,
    startIndex = 23,
    axisLabel = "date",
    labels = vec,
    updateInterval = 5,
    playIcon = "fas fa-play",
    pauseIcon = "fas fa-pause",
    magnet = list(
      rounc = "floor",
      step = 0.1
      )
  ) |>
  hc_legend(layout = "vertical", verticalAlign = "top",
            align = "right", valueDecimals = 0) |>
  hc_mapNavigation(
    enabled = TRUE,
    enableMouseWheelZoom = TRUE,
    enableDoubleClickZoom = TRUE
  ) |>
  hc_credits(enabled = TRUE,
             text = last_updated,
             position = list(align = "left", x = 10, y = -5))


frameWidget(widget, width="100%", height="40rem")</code></pre>

# Further analysis

Aside from drilling into other categories of data here and checking against other data sets, I'll also be looking at various forecast models and lag indicators. There will also be *drill down* into national and regional level data. In any case, as the story continues to unfold we should all brace for a very *dark* winter.
