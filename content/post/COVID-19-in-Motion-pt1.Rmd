...
data <- select(coviddata[with(coviddata, order(date, iso_code)),], date, continent, new_cases_smoothed_per_million)   %>%
...
So I took the data and plotted it on a map, to play on a day grain through time. The gradations are pretty subtle at the start, and that's because of the data getting *very* dark late in the range. In fact when I first created this report I didn't have 
...
`r blogdown::shortcode("alert", "info", "Working with the Report:", "Aside from the play/pause and controls, the map is *zoomable*. Hovering over a country will highlight and show the value for the current position on the timeline. Hovering while playing back will show rolling changes to the values for that country.")`

`r blogdown::shortcode("alert", "warning", "Bug Report:", "The play/pause button for the map charts *works* but the icons are currently not visible. A fix is being researched now, but thought the power of the visual was worth publishing with missing key element. Thanks for understanding.")`

# Daily COVID Mortality

One of the things I expected was to see some "chop" in early data as reporting became more consistent. Here I was surprised to see irregular bumps in the "smoothed" data. I'm not sure whether it's *my processing* of the data (grouped/summed by continent) or if there's something in the data itself. That's something I'll have to check against other data sets and within my code. Such as it is, the visuals tell a convincing story of where things are going as of November, and it's *not* good.
...
Again here - like the area chart - the direction of what's happening in Europe is unmistakable. In fact it bothers me to the point that I'll probably end up sourcing some other data just so I can sleep tonight. 
...
data <- select(coviddata[with(coviddata, order(date, iso_code)),], date, continent, new_tests_smoothed)   %>%
  rename(value = new_tests_smoothed) %>%
  mutate(value = ifelse(is.na(value), 0, value)) %>%
  mutate(date = ymd(date)) %>%
  group_by(date, continent) %>%
  summarise_each(funs(sum))
...
widget <- hchart(data, "area", hcaes(x = date, y = value, group = continent))%>%
  hc_plotOptions(series = list(stacking = 'normal')) %>%
  hc_title(text = "Global COVID Daily Tests (smoothed)", align = "center") %>% 
  hc_add_theme(thm) %>%
...
data <- select(coviddata[with(coviddata, order(date, iso_code)),], date, iso_code, location, new_tests_smoothed_per_thousand)  %>%
