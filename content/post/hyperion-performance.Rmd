---
title: "Tracking Global Hyperion EPM Performance"
output:
  blogdown::html_page:
    toc: true
    toc_depth: 1
...
cover: /img/world-map-black.png
description: Using PowerBI to create a Lamda-architecture "speed layer" monitor for Hyperion Finance activity 
draft: no
tags:
- Azure
- Oracle
...
---
```{r setup and file load, include=FALSE}
...
concurrencyTable <- fromJSON("../../data/Concurrency.json")
dfConcurrency <- as.data.frame(do.call("cbind", concurrencyTable))

dfConcurrency$Consolidation = (as.integer(dfConcurrency$Consolidation) - 1)
dfConcurrency$LargeSMV = (as.integer(dfConcurrency$LargeSMV) - 1)
dfConcurrency$TBUploads = (as.integer(dfConcurrency$TBUploads) - 1)
```

##  `r fa("readme")` tl:dr
<div class="clearfix"><img class="ui medium left floated image" src="/img/oracle_hyperion.png">The term “performance” can mean different things within a given corporate context. In financial firms it most often refers to profit or loss. In tech companies, it can refer to throughput of an application or service. In this case, the term “performance” shares both of those connotations. The company was upgrading their Oracle Hyperion Financial Management installation, which is used to measure the company’s financial performance (and supports other tasks, such as forecasting). And because of the global nature of the company, and reliance on detailed ledger data ETL’d in from remote manufacturing locations, there was also a system-level performance aspect to the assignment. Due of the critical nature of close period reporting, there was a high degree of sensitivity around timely submission of data. This entry in my portfolio will focus on that latter portion of the “performance” equation, though the implications to the former will become clear as this entry progresses.</div>

# `r fa("globe")` Beginning With The End In Mind

<center>
<iframe width="940" height="600" src="https://app.powerbi.com/view?r=eyJrIjoiNTljZTM0ZGUtNTk4Ni00MjMxLWE1ZTYtYjJiODJiMWU4ODU5IiwidCI6Ijc4N2QxYzAwLTgyMjgtNDAzYy1iZjVmLTg1YjM3NzFmM2M0MiIsImMiOjZ9/" frameborder="0" allowFullScreen="true"></iframe>
</center>
...
<img class="ui centered rounded bordered image" src="/img/ScreenShot2016-09-21at8.12.02PM.png">
...
## `r fa("calendar-alt")` Challenge 3: Accelatis time stamps (.NET and Unix issues)
...
<img class="ui centered rounded bordered image" src="/img/SSIS_flat-1.png">

## `r fa("calendar-alt")` Staging tables – Agent data
...
<img class="ui centered rounded bordered image" src="/img/image011.png">
...
## `r fa("clipboard-list")` Data Tables – Consolidations
...
<img class="ui centered rounded bordered image" src="/img/image012.png">

## `r fa("microchip")` Data Tables – HFM and DB CPU
...
<img class="ui centered rounded large bordered image" src="/img/image013.png">
...
```{r "Concurrency of Hyperion Processes", fig.align="center"}
series = list(
  list(
    name = 'Consolidations',
    data = dfConcurrency$Consolidation
  ),
  list(
    name = 'Large SmartViews',
    data = dfConcurrency$LargeSMV
  ),
  list(
    name = 'Ledger Uploads',
    data = dfConcurrency$TBUploads
  )
)

hcConcurrency <- highchart() %>%
  hc_title(text = paste("Concurrency of Long-Running Hyperion Processes")) %>% 
  hc_chart(
      type = 'area',
...
#      borderColor = 'black',
  ...
  hc_xAxis(categories = dfConcurrency$DateTime) %>% 
  hc_yAxis(allowDecimals = 'false') %>%
  hc_add_series_list(series) %>%
  hc_plotOptions(
    series = list(
      stacking = 'normal', marker = 'none', step = 'both')) %>% 
  hc_add_theme(hc_theme_gridlight())

frameWidget(hcConcurrency)
```

This is the portion of the process which has the most technical “inner workings”. Up to this point SSIS has been used mainly for parameter substitution to collect data through queries. This section will show how C# scripting is used to construct the concurrency table.

<center>
```{r "Data Table for Hyperion Concurrency"}
dtConcurrency <- datatable(dfConcurrency, options = list(
  initComplete = JS(
    "function(settings, json) {",
    "$(this.api().table().header()).css({'background-color': '#000', 'color': '#fff'});",
    "$(this.api().table().row()).css({'background-color': '#000', 'color': '#fff'});",
    "}")
))

frameWidget(dtConcurrency)
```
</center>
...
<pre><code class="c#">for (DateTime date = CurrentRow; CurrentRow <= PeriodEnd; CurrentRow = CurrentRow.AddSeconds(1.0)) {
...
From that point it’s simply a matter of charting those values in visualization tools such as PowerBI or HighCharts.js. The latter example can be seen at the top of this section, and PowerBI is used in the next chart.

# `r fa("chart-line")` Visualization, Part 1
...
##  `r fa("stopwatch")` Derivation of maximum reasonable response times for SLAs
...
<img class="ui centered rounded bordered image" src="/img/image020.png">
...
<img class="ui centered rounded  bordered image" src="/img/image021.png">
...
<img class="ui centered rounded bordered image" src="/img/image022.jpg">
...
##  `r fa("chart-area")` Analysis – using PowerBI to review error incident rates by location and type
...
<center>
<iframe width="940" height="600" src="https://app.powerbi.com/view?r=eyJrIjoiMDI4MDdlNzktZTNmYi00ZWEwLWFiMjktM2IyMWQ1MmYyNTA0IiwidCI6Ijc4N2QxYzAwLTgyMjgtNDAzYy1iZjVmLTg1YjM3NzFmM2M0MiIsImMiOjZ9" frameborder="0" allowFullScreen="true"></iframe>
</center>
...
# `r fa("diagnoses")` Visualization, Full Circle
...
<center>
<iframe width="940" height="600" src="https://app.powerbi.com/view?r=eyJrIjoiNjAyZjYzYzctN2M2Ni00YTYxLTgwODktMjVjMmUxNzllZTMzIiwidCI6Ijc4N2QxYzAwLTgyMjgtNDAzYy1iZjVmLTg1YjM3NzFmM2M0MiIsImMiOjZ9" frameborder="0" allowFullScreen="true"></iframe>
</center>
