---
title: "COVID-19 in Motion - Israel as Case Study"
output:
  blogdown::html_page:
    dev: "svg"
...
```{r Israel COVID Cases per Million, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
thm <- hc_theme_merge(
  hc_theme_smpl(),
  hc_theme(colors = c("#800000", "#102156", "#d4622b",  "#45C0F5", "#AFDC00", "#85DFD0", "purple", "orange"),
...
```{r Israel COVID Deaths per Million, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
widget <- highchart(type = "stock") %>%
  hc_add_theme(thm) %>%
  hc_title(text = "Global New Deaths per Million - Israel as Case Study", align = "center") %>%
  hc_subtitle(text = "With Comparison to Japan, China, India, Canada, Germany, UK & US", align = "center") %>%
    hc_chart(
      borderColor = 'rgba(160, 160, 160, 0.3)',
      borderRadius = 8,
...
```{r Israel COVID Tests per Thousand, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
widget <- highchart(type = "stock") %>%
  hc_add_theme(thm) %>%
  hc_title(text = "Global New Tests per Thousand - Israel as Case Study", align = "center") %>%
  hc_subtitle(text = "With Comparison to Japan, China, India, Canada, Germany, UK & US", align = "center") %>%
    hc_chart(
      borderColor = 'rgba(160, 160, 160, 0.3)',
      borderRadius = 8,
...
The file from "Our World in Data" is pulled in a single chunk at the top of the R markdown document, and the entire data set is placed in a dataframe that each individual report chunk can process for its own use. You'll see the creation of an initial *data* dataframe, which in each case selects on of the dozen or more columns in the original set. Then the *wide* version of the table is created to split out each country-based time series into its own column. That *wide* dataframe is then converted to an *xts* or extenteded time series dataframe.

The *plotLines* and *plotBands* are largely manually generated - from dates I found in the wikipedia page mentioned above. The one date that is dynamically set is the end date for the plotBand which always grabs the last date of the current report range. Once there's a change in policy then that will again require some hand-curated edits, as opposed to the general daily pull of data for the time series information.
...
widget <- highchart(type = "stock") %>%
  hc_add_theme(thm) %>%
  hc_title(text = "Global New Cases per Milion - Israel as Case Study", align = "center") %>%
  hc_subtitle(
