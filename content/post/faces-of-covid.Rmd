...
date: '2018-11-07T14:59:43+05:30'
featuredpath: img
...
The "get_timeline" command first takes the account name (which can also be an account id) and an value of 1,000 - to get a longer list that then default 100. Filtering significantly pares down the list and getting 1,000 tweets mean I go "back in time" far enough to get a good cross-section from which to randomly select the posts to embed. The *parse* function does the heavy lifting and putting the API result into a dataframe, and setting *include_rts* to FALSE keeps retweets out of the list. And finally, the filter command selects for posts containing a string that I use a *key* to strive to get announcement posts. In my last check that pared down the initial 1,000 to slightly less than 400 tweets, which is plenty for the next step.
...
And it's certainly possible to keep going. Notice here how I simply created a single "row" with multiple tweets that in standard landscape orientation the browser displays in a 3X3 grid.
...
Aside from the normal compose/fix/build/deploy cycle in fleshing out this site, the build/deploy cycle also re-triggers whenever there's new COVID data available. I'll be writing about that under separate cover but it bears mentioning here to illustrate how that process also benefits this layout function. As the COVID data is refreshed and site re-deployed, it also re-runs the rtweet functions. That will not only gather any new posts that have made over the previous day, but will continue to shuffle and display "new" entries. So it will keep rotating the tweets and continue to shine a light on the myriad life stories that were in so many cases unduly cut short by this on-going tragedy.
