...
The end result is elegant and cheap, but getting this side projet to a notch point was not particularly straightfoward. Once I figured out how the message was to be parsed for the SendGrid v3 API along with a few tweaks applied to the Function App settings things are slick and quick. I added some JavaScript and CSS to annimate the Azure Function Apps logo to provide a more active user experience. For the full project and sample HTML and JS files (shown below) you can proceed the GitHub repo `r blogdown::shortcode("url-link", "here", "https://github.com/houstonhaynes/ContactFunction")` and a GIF of the form in action is shown below.
...
The next two blocks of code are ported from a gist because the raw embeds as above don't go so well with raw html and JSON. That's something I'll look into but for now you should get a picture - even with GitHub's ugly, nearly useless highlighting.
