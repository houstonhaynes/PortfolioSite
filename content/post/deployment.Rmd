...
I've long held (as in **for _<a href="/vitae/about/#IBM">decades</a>_**) that a software engineer could only own the "full stack" moniker if they had experience with instrumenting automated deployment. Over the past few years "devops" has become all the rage, and I'm *here for it*. This is the first of three articles outlining how I build and migrate this site. This entry will focus on the build internals, which is likely to *remain* self-hosted for a few reasons I outline below. 

The second post will focus on the Github Actions self-hosted runner implementation, and how that's triggered from my private GitHub repo. The third will look at how I use this process to re-deploy the site based on updates to source data that are used for time series visualizations on this site. Since this is a *static* site I can't necessarily update when my source data updates, but there are a few *cheat* mechanisms that, with this hands-free deployment process in place, will let me re-trigger the deployment and use R to collect the fresh data at build time. This will allow me to keep the site *and its underlying data* up-to-date without having to chase down loose ends myself.
...
This deserves a bit of unpacking because of the "Russian doll" effect. This is called from within a headless command prompt *within* the PowerShell script, and the R script runs a verbose <a href="https://bookdown.org/yihui/blogdown/methods.html" target="_blank">blogdown</a> command which calls Hugo to generate the site.
...
This is not the standard "blogdown::build_site()" command because there's a wrinkle in the config.toml that I use for local debug. Below is an excerpt from the top of my site's config file.
...
These lines do the due diligence of reading the properties entries from the JSON file.
...
This is a housecleaning move that clears everything from the public folder. (the destination that Hugo uses to output all generated web site assets)
